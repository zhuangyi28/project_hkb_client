...
var hkbpswKeybroad ={
    ...
    keyBroadShow:function(details){
        ...
        _this.Fn = details.Fn || 0;//输入六位密码后所做的操作
        ...
        _this.run=function(){
            ...
            //插入html部分
            var divInner='<div class="weui_actionsheet" id="weui_actionsheet_keyboard"><div class="weui_grids weui_grids_kj">'+thisKeybroadNum+'<a class="weui_grid weui_grid_0" data-value="0"><p class="weui_grid_label">0</p></a><a class="weui_grid" id="hkb_actionsheet_delete"><p class="weui_grid_label"><span class="actionsheet_delete_icon"></span></p></a>';

            if(!document.getElementById('hkb_pw_keybroad')){//如果当前页面中没有键盘，则添加元素，键盘移除，所有绑定事件取消

                addEle("div", 0, "hkb_pw_keybroad","hkb_pw_keybroad", divInner);

                setTimeout(function(){

                    document.getElementById('hkb_pw_keybroad').getElementsByClassName('weui_actionsheet')[0].className="weui_actionsheet"
                },10);

                setTimeout(function(){

                    document.getElementById('hkb_pw_keybroad').getElementsByClassName('weui_actionsheet')[0].className="weui_actionsheet weui_actionsheet_toggle"
                },30);
                ...
                /*键盘数字监听，添加金额*/
                for(var i=0;i<thisAllNum.length-1;i++){
                ...
                /*删除事件*/
                document.getElementById('hkb_actionsheet_delete').addEventListener("touchstart",DeleteThisEle,false);//键盘上的删除键
            ...
            function getALLNum(obj) {
                ...
                /*密码转化为●的方法*/
                var square = document.getElementById('pasdBox').getElementsByTagName('span');

                var inputValue = document.getElementById('pasd_input').value;

                for (var i=0;i<thisInnerNum.length+1;i++){

                    if(thisInnerNum.length < 6){
            ...
            function DeleteThisEle(){
                ...
                function deleteCircle() {
                    ...
                    for(var j=0;j<thisInnerNum.length;j++){
        ...
        _this.stop=function(){
            ...
            if(document.getElementById('hkb_pw_keybroad')){/*当前键盘存在*/

                document.getElementById('hkb_pw_keybroad').getElementsByClassName('weui_actionsheet')[0].className="weui_actionsheet";//键盘先收回去
                ...
                setTimeout(function(){
                    ...
                    removeEle(document.getElementById('hkb_pw_keybroad'));//删除键盘
...
var passWordBox = {
    ...
    //密码box出现
    passWordBoxShow:function () {

        var thisEle = document.getElementsByClassName('pay_pasd_box')[0];

        var thisBg = document.getElementsByClassName('box_gray')[0]
        ...
        thisBg.addEventListener('touchmove', windowBanEvent.Canceling);//给阴影绑定禁止事件


        //hkbkeybroad.run();//页面初始的时候，开启键盘

    },

    //密码box消失
    passWordBoxHide:function () {
...
/*function hkbKeyBoardPw(e) {


 var numEle = document.getElementById('hkb_pw_keybroad').getElementsByClassName('weui_grids_kj')[0];

 numEle.addEventListener('click', passWordMake, false);

 numEle.addEventListener('click', passWordShow,false);
 ...
 if (thisTargetEle != document.getElementById('hkb_actionsheet_delete')) { //如果点击到的不是删除键

 numBox.innerHTML = numBox.innerHTML + thisTargetEle.getAttribute('data-value');

 }
 else {

 var num = document.getElementById('numBox').innerHTML;
 ...
 var numbox = document.getElementById('numBox').innerHTML;
